...
from usuarios u left join "db-stage-prod"."interactions_cwp" i on users = i.account_id left join "db-stage-dev"."so_hdr_cwp" s on users =  cast(s.account_id as VARCHAR) and date_trunc('MONTH',date(u.dt)) = date_trunc('MONTH',date(i.dt))
