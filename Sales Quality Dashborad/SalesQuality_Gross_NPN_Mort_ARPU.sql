...
--select * from summary_by_user limit 100

-- ######################################## Mortlity Rate ###############################################

,forward_months as (
Select date_trunc('MONTH', date(dt)) as month_survival,dt, act_acct_cd, fi_outst_age
from "db-analytics-prod"."fixed_cwp" 
...
max(case when month_forward = (select input_month from parameters) + interval '12' month then rgu_cnt else null end ) as rgu_12
...
,join_mrc as (
select a.*,b.*
from join_new_acct_surv a inner join mrc_evo b on a.act_acct_cd = b.act_acct_cd
)

select * from join_mrc
