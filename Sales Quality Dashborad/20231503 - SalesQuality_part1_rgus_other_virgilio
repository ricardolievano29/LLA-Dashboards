--CREATE TABLE IF NOT EXISTS "lla_cco_int_san"."cwp_sales_digital_dec"  AS 

WITH 
Parameters AS (
SELECT 
DATE('2022-12-01') AS input_month,
date('2023-03-13') as current_month
 )
--###################### INFORMACIÓN GENERAL ####################################
...
SELECT 'Upsell' AS base_classification, upsell_type,act_acct_cd,first_pd_mix_qty,first_rgu_qty,last_pd_mix_qty,last_rgu_qty,max_rgu as last_rgu, upsell_rgus,upsell_rgu_with_planchanges,upsell_rgus_names,
        --upsell_update_date, 
        first_bill_after_upsell,--NULL AS first_installation_date,
...
SELECT 'Gross-add' AS base_classification, NULL AS upsell_type, act_acct_cd, null as first_pd_mix_qty,null as first_rgu_qty,null as last_pd_mix_qty,null as last_rgu_qty,max_rgu as last_rgu,NULL AS upsell_rgus,null as upsell_rgu_with_planchanges,
        ...
        NULL AS first_bill_after_upsell,-- first_installation_date,
...
,gross_adds_view as (
select distinct 
sales_month,act_acct_cd,techflag,bunddle_type,socioeconomic_seg,geography,first_rgu_cnt,last_rgu_cnt,rgus_sold,rgu_vendidos, movement_flag,fi_sales_channel as sales_channel, case when fi_sales_channel = 'Contractors' then fi_sales_channel_sub else null end as contractor, fi_codigo_de_vendedor as cod_sales_rep,npn_30_flag,npn_60_flag,npn_90_flag,npn_flag,window_completed_30 ,window_completed_60 ,window_completed_90,first_bill_created,first_bill_after_upsell,last_rgu,last_pd_mix_qty,first_pd_mix_qty
from summary_flag
)


select act_acct_cd,movement_flag,last_pd_mix_qty,first_pd_mix_qty,rgus_sold,rgu_vendidos,first_bill_after_upsell, case when rgu_vendidos > rgus_sold then 'Grater Virigilio' else 'Grater Oval' end as rgus_comparison from (select distinct *,row_number() over (partition by sales_month,act_acct_cd order by techflag) as duplicates from gross_adds_view) where duplicates = 1 and rgu_vendidos <> rgus_sold and movement_flag in ('Upsell')
