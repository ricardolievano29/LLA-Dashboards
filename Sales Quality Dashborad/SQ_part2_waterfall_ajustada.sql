...
Parameters AS (
SELECT 
DATE('2022-04-01') AS input_month,
date_trunc('Month' ,current_date) as current_month
)

,part_one as (
select * from "lla_cco_int_san"."cwp_sales_quality_apr22"
)
-- ######################################## Mortlity Rate ###############################################
...
from  "db-analytics-prod"."fixed_cwp" 
WHERE  CAST(act_acct_cd AS BIGINT) in (select cast(act_acct_cd as bigint) from mortality_view) and date(dt)< (select input_month from parameters) + interval '7' month
)
