...
-- select count(distinct late_inst_flag),count(distinct early_ticket_flag),count(distinct early_interaction_flag) from change_mrc_flag 
------------------------ Mounting Bills ----------------------------------
,overdue_records AS (
...
SELECT  case when month is null then month_b else month end  as month
--/*
        -- ,B_Final_TechFlag
        -- ,B_FMCSegment
        -- ,E_Final_TechFlag
...
GROUP BY 1--,3,4,5,6,7,8,9,B_FMCTYPE, E_FMCTYPE ,Days_flag_30,Days_flag_60,Days_flag_90 first_sales_chnl_eom, first_sales_chnl_bom, Last_Sales_CHNL_EOM, Last_Sales_CHNL_BOM , sales_channel,sales_channel_so
ORDER BY 1
